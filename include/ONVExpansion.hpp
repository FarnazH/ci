 ...
 */
template <>
class ONVExpansion<boost::dynamic_bitset<>> {
private:
    std::vector<ci::ONVExpansionComponent<boost::dynamic_bitset<>>> expansion;
    size_t K;  // number of spatial orbitals
    size_t dim;  // the dimension of the expansion
...
public:
     ...
     *  Constructor based on a given initializer list
     */
    ONVExpansion(const std::initializer_list<ci::ONVExpansionComponent<boost::dynamic_bitset<>>>& list) :
        expansion (list),
        K (this->expansion[0].alpha.get_K()),
        dim (this->expansion.size())
    {
    ...
    explicit ONVExpansion(const std::string& GAMESS_filename) {
        ...
        while (std::getline(input_file_stream, line)) {
            ...
            std::string reversed_alpha (trimmed_alpha.rbegin(), trimmed_alpha.rend());
            bmqc::SpinString<boost::dynamic_bitset<>> alpha ((boost::dynamic_bitset<> (reversed_alpha)));
            ...
            std::string reversed_beta(trimmed_beta.rbegin(), trimmed_beta.rend());
            bmqc::SpinString<boost::dynamic_bitset<>> beta ((boost::dynamic_bitset<> (reversed_beta)));
            ...
            // Add an ONVExpansionComponent to the end of this
            this->expansion.emplace_back(ci::ONVExpansionComponent<boost::dynamic_bitset<>>{alpha, beta, coefficient});
        }  // while getline
    ...
    explicit ONVExpansion(const ci::FCI& fci) {
        ...
        // The convention that is used is that the alpha addresses are major, i.e. the beta addresses are contiguous
        //      I_alpha I_beta = I_alpha * dim_beta + I_beta
        bmqc::SpinString<boost::dynamic_bitset<>> spin_string_alpha (0, fci.get_addressing_scheme_alpha());
        for (size_t I_alpha = 0; I_alpha < fci.get_dim_alpha(); I_alpha++) {
            if (I_alpha > 0) {
                spin_string_alpha.nextPermutation();
            }

            bmqc::SpinString<boost::dynamic_bitset<>> spin_string_beta (0, fci.get_addressing_scheme_beta());
            for (size_t I_beta = 0; I_beta < fci.get_dim_beta(); I_beta++) {
                if (I_beta > 0) {
                    spin_string_beta.nextPermutation();
                }
                ...
                this->expansion.emplace_back(ci::ONVExpansionComponent<boost::dynamic_bitset<>> {spin_string_alpha, spin_string_beta, coefficient});
            }  // I_beta loop
        }  // I_alpha loop
    }
    ...
    explicit ONVExpansion(const ci::DOCI& doci) {
        ...
        // Emplace-back this with the alpha-ONVs, beta-ONVs (which are equal to the alpha-ONVs) and coefficients
        bmqc::SpinString<boost::dynamic_bitset<>> spin_string(0, doci.get_addressing_scheme());
        for (size_t I = 0; I < doci.get_dim(); I++) {
            if (I > 0) {
                spin_string.nextPermutation();
            }
            ...
            this->expansion.emplace_back(ci::ONVExpansionComponent<boost::dynamic_bitset<>>{spin_string, spin_string, coefficient});
        }  // loop over all addresses I
    }
     ...
     *  OPERATORS
     */
    ci::ONVExpansionComponent<boost::dynamic_bitset<>> operator[](size_t index) const {
        return this->expansion[index];
    }


    friend std::ostream& operator<<(std::ostream& os, const ci::ONVExpansion<boost::dynamic_bitset<>>& expansion) {
        for (const auto& component : expansion.expansion) {
            os << component << std::endl;
        }
        return os;
     ...
     *  GETTERS
     */
    std::vector<ci::ONVExpansionComponent<boost::dynamic_bitset<>>> get_expansion() const { return this->expansion; }
     ...
     *  @return if this is equal to @param other, within a given @param tolerance
     */
    bool isEqual(const ci::ONVExpansion<boost::dynamic_bitset<>>& other, double tolerance=1.0e-12) const {
    ...
    void calculate1RDMs() {
        ...
        for (size_t I = 0; I < this->dim; I++) {  // loop over all addresses (1)
            ...
            bmqc::SpinString<boost::dynamic_bitset<>> alpha_I = this->expansion[I].alpha;
            bmqc::SpinString<boost::dynamic_bitset<>> beta_I = this->expansion[I].beta;
            double c_I = this->expansion[I].coefficient;
            ...
            for (size_t J = I+1; J < this->dim; J++) {
                ...
                bmqc::SpinString<boost::dynamic_bitset<>> alpha_J = this->expansion[J].alpha;
                bmqc::SpinString<boost::dynamic_bitset<>> beta_J = this->expansion[J].beta;
                double c_J = this->expansion[J].coefficient;
