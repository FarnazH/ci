...
class ONVExpansion {
private:
    const std::vector<ci::ONVExpansionComponent<T>> expansion;
...
//            if (line.find("ALPHA") != std::string::npos && line.find("BETA") != std::string::npos &&
//                line.find("COEFFICIENT") !=
//                std::string::npos) {  // if find returns an index that's different from the 'not-found' index
...
//                    "One of the provided alpha ONVs does not have the correct number of orbitals.");
//            }
...
//                throw std::invalid_argument(
//                    "One of the provided beta ONVs does not have the correct number of orbitals.");
//            }
...
//    }
    ...
    // OPERATORS
    ci::ONVExpansionComponent<T> operator[](size_t index) {
