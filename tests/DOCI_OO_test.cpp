...
BOOST_AUTO_TEST_CASE ( OO_DOCI_h2o_STO_3G ) {
    ...
    // Do the DOCI orbital optimization, using the a random unitary transformation of the RHF orbitals
    Eigen::MatrixXd A = Eigen::MatrixXd::Random(so_basis.get_K(), so_basis.get_K());
    Eigen::MatrixXd Sym = A + A.transpose();
    Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> saes (Sym);
    so_basis.rotate(saes.eigenvectors());

    ci::DOCI doci (so_basis, h2o);

    std::cout << "Maximum Shannon entropy: " << std::log(doci.get_dim()) / std::log(2) << std::endl;
    ...
    std::cout << std::setprecision(15) << doci.get_eigenvalue() << std::endl;
}
